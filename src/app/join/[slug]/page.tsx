...
export default function JoinPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    if (!challenge.allowed_countries?.includes(form.country)) { toast.error('Sorry, your country is not eligible for this challenge'); return }
    ...
    <div className="min-h-screen bg-white dark:bg-lpr-black px-4 py-8">
      <div className="max-w-lg mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {challenge.prize_pool?.[0] && (
              <div className="bg-amber-500/10 rounded-xl p-3">
                ...
                <p className="font-bold text-amber-500">${challenge.prize_pool[0].amount} NZD</p>
              </div>
            )}
            <div className={`rounded-xl p-3 ${isFull ? 'bg-red-500/10' : 'bg-green-500/10'}`}>
              <p className="text-gray-400 text-xs">Spots</p>
              <p className={`font-bold ${isFull ? 'text-red-400' : 'text-green-500'}`}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {challenge.allowed_countries.map((c: string) => (
                  <span key={c} className="text-xs bg-cobalt-500/10 text-cobalt-400 px-2 py-1 rounded-full">
                    {COUNTRY_FLAGS[c] || '🌏'} {c}
                  </span>
                ))}
              </div>
        ...
        {isFull ? (
          ...
          <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} onSubmit={handleJoin} className="card space-y-4">
            <h2 className="font-bold dark:text-white text-lg">Join the Challenge</h2>
            <div><label className="label">Full Name</label><input required className="input" placeholder="Robin Leaton" value={form.fullName} onChange={e => setForm(p => ({ ...p, fullName: e.target.value }))} /></div>
            <div><label className="label">Email</label><input required type="email" className="input" placeholder="robin@example.com" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} /></div>
            <div><label className="label">Password</label><input required type="password" className="input" placeholder="Choose a password" minLength={6} value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} /></div>
            <div>
            ...
            <div className="space-y-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={form.agreeSteps} onChange={e => setForm(p => ({ ...p, agreeSteps: e.target.checked }))} className="mt-0.5 accent-cobalt-500" />
                <span className="text-sm text-gray-600 dark:text-gray-300">I understand that steps must be submitted before <strong>11:59pm on the same day</strong> or they cannot be counted.</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={form.agreeFeedback} onChange={e => setForm(p => ({ ...p, agreeFeedback: e.target.checked }))} className="mt-0.5 accent-cobalt-500" />
                <span className="text-sm text-gray-600 dark:text-gray-300">I'm happy to provide <strong>feedback at the end of the challenge</strong> to help improve future challenges.</span>
              </label>
            </div>
            <button type="submit" disabled={submitting} className="btn-primary w-full flex items-center justify-center gap-2">
              {submitting ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Joining...</> : '🏃 Join Challenge'}
            </button>
          </motion.form>
        )}
      </div>
